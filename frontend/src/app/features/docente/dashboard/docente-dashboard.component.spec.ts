...
class ReunionesServiceStub {
  ...
  constructor() {
    const baseResuelta: SolicitudReunion = {
      ...
      proyectoNombre: 'Proyecto Carlos',
  ...
  aprobarSolicitud(id: number, payload: any) {
    return of({
      ...
      proyectoNombre: this.solicitudes.find((item) => item.id === id)?.proyectoNombre ?? null,
      docente: null,
      solicitudId: id,
      trazabilidad: [],
    });
